...
export default function JobDetailPage() {
  ...
  // Invoiced-edit guard — must be called after job state is available
  ...
  const saveServices = () => {
    ...
    guardedAction(async (changeReason) => {
      ...
      try {
        ...
        // Re-throw so GuardModal can surface backend errors inside itself
        throw err;
      } finally {
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        {/* Invoice-regen warning — shown only when job is Invoiced */}
        {job.status === "Invoiced" && (
          <div className="bg-amber-950/40 border border-amber-700/60 rounded-lg px-4 py-3 text-sm text-amber-300 flex flex-col sm:flex-row sm:items-center gap-2">
            <span className="font-medium">This job has been invoiced.</span>
            <span className="text-amber-400/80">
              Edits won&apos;t update the sent invoice — regenerate the invoice if amounts changed.
            </span>
            {invoiceInfo && (
              <a
                href={invoiceInfo.url}
                target="_blank"
                rel="noopener noreferrer"
                className="underline text-gold hover:text-gold-light font-medium whitespace-nowrap"
              >
                Regenerate invoice
              </a>
            )}
